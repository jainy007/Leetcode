...
#include <iostream>
#include <vector>
#include "Graphs/DisjointSets/src/lib/DFS/allPaths/dfsAllPaths.h"
...
TEST(dfsallPathsTest, ReturnAllPaths) {
    Dfs dfs;
    dfsallPaths d;
    ...
    actualOutput1 = dfs.allPaths(graph1);
    EXPECT_EQ(output1.size(), actualOutput1.size());

    actualOutput2 = dfs.allPaths(graph2);
